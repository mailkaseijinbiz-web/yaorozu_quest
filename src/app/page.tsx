...
export default function HomePage() {
  ...
  const swipeIdx = SWIPE_TABS.indexOf(activeTab);
  // 横スワイプ中は縦スクロールをロック（フック側で touchmove を preventDefault）。
  const swipeRef = useSwipeNav<HTMLDivElement>({
    enabled: swipeIdx >= 0,
    allowLeft: swipeIdx >= 0 && swipeIdx < SWIPE_TABS.length - 1, // 次のタブがある
    allowRight: swipeIdx > 0,                                     // 前のタブがある
    onCommitLeft: () => { const i = SWIPE_TABS.indexOf(activeTab); if (i >= 0 && i < SWIPE_TABS.length - 1) setActiveTab(SWIPE_TABS[i + 1]); },
    onCommitRight: () => { const i = SWIPE_TABS.indexOf(activeTab); if (i > 0) setActiveTab(SWIPE_TABS[i - 1]); },
  });
    ...
    <div className="flex-1 min-h-dvh bg-[#eaecef] flex items-center justify-center font-sans overflow-hidden p-0 sm:p-4 md:p-8 relative">
      ...
      <div className="w-full h-dvh sm:h-[840px] sm:max-w-[395px] sm:rounded-[48px] sm:border-[11px] sm:border-[#1E2024] sm:shadow-[0_24px_80px_rgba(0,0,0,0.15)] relative overflow-hidden flex flex-col bg-[#f5f7fa] z-10 sm:scale-[0.98] lg:scale-100 transition-all duration-300">
        ...
        <div className="flex-1 relative overflow-hidden bg-[#f5f7fa] flex flex-col">
          ...
          {/* Tab content（外側=指追従でスライドする安定要素 / 内側=タブ切替のクロスフェード） */}
          <div ref={swipeRef} className="flex-1 h-full overflow-hidden relative z-0">
          <div
            key={activeTab}
            className={`h-full w-full overflow-hidden animate-in fade-in ${activeTab === 'home' ? '' : 'overflow-y-auto'}`}
          >
