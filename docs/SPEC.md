...
- **スポットマーカー**: divIcon バブル。フキダシは選択中の場に対して表示（青丸マーカーの上に開く）。無選択時のみ最寄りの場に表示。🦊 神絵文字 + 30字に切り詰めた神の声を白バブルで表示（4.5秒間隔で回転、フェード）。それ以外は**種別アイコン**（神社=⛩️ / 寺院=🙏）で表示し、地図上で寺社の種別を一目で見分けられる。種別アイコン・選択中の青丸の下には名前キャプション（`.map-spot-name`、白フチの Google Map ラベル風）を表示する。
